...
def start_command(message):
    ...
    bot.send_message(
        message.chat.id, 
        "Привет, пришли куки человека которого хотите взломать, мы его рефрешнем и передадим вам🍪 ❗НЕ НУЖНО КИДАТЬ НИК ЖЕРТВЫ, ПОСМОТРИТЕ ДОСТАТОЧНО ВИДЕО ТУТОРИАЛ НА НАШЕМ КАНАЛЕ - @s1iuy❗"
    )
    try:
